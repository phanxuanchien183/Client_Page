...
const BannersPage = () => {
  ...
  //? Render(s)
  const title = category_name ? `${trans.common.banner_management} - ${category_name}` : trans.common.banner_management
  useTitle(title)
  const renderContent = () => {
    ...
    if (banners?.data && banners?.data?.length > 0) {
      return banners?.data.map(banner => (
        ...
        >
          ...
          <td className="px-2 py-4">
            <Link
              href={`/admin/banners/edit/${banner.id}?banner_name=${banner.title}`}
              className="bg-rose-50 text-rose-500 rounded-sm py-1 px-1.5 mx-1.5 inline-block"
            >
              {trans.banner.edit}
            </Link>
            ...
            <table className="w-full whitespace-nowrap">
              <thead className="h-9 bg-emerald-50">
                <tr className="text-emerald-500">
                  {category_name && <th className="border-gray-100 border-x-2">{trans.common.image}</th>}
                  <th className="px-2 border-gray-100 border-x-2">
                    {category_name ? trans.banner.title : trans.category.name}
                  </th>
                  {category_name && <th className="border-gray-100 border-x-2">{trans.common.type}</th>}
