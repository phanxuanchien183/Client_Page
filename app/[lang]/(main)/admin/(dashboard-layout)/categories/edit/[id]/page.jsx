...
import { useDisclosure, useTitle, useUrlQuery, useTranslate } from '@/hooks'
...
export default function CategoriesEditPage({ params: { id } }) {
  ...
  const { isLoading: isLoading_get, selectedCategory } = useGetCategoriesQuery(undefined, {
    selectFromResult: ({ data, isLoading }) => ({
      selectedCategory: data?.data?.categories.find(category => category.id === id),
      isLoading,
    }),
  })
  ...
  return (
    <>
      ...
      />
      <main>
        <PageContainer title={trans.category.edit}>
          {isLoading_get ? (
