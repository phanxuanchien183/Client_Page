...
const BasicAddresses = ({ addressModalProps }) => {
  ...
  return (
    <main>
      <PageContainer title="Sổ địa chỉ">
        {isLoading ? (
            ...
            <div className="my-2 space-y-3 text-gray-500">
              <div className="flex items-center gap-x-2 ">
                ...
                <span className="text-xs md:text-sm">
                  {address?.province?.name}{address?.city ? `, ${address?.city.name}`: ''}{address?.area ? `, ${address?.area.name}` : ''}
                </span>
              </div>
              <div className="flex items-center gap-x-2 ">
                <Icons.Post className="text-gray-500 icon" />
