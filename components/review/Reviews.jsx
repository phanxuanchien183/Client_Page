    ...
    <>
      ...
      <section className="px-3 py-3 space-y-4 lg:max-w-3xl xl:max-w-5xl" id="_productReviews">
        <div className="flex items-center justify-between">
          <h4 className="mb-3 lg:border-b-2 lg:border-red-500">Đánh giá sản phẩm</h4>
          <span className="text-xs text-sky-500">{numReviews} lượt đánh giá</span>
        </div>
        <div className="lg:ml-10">
          <div className="mb-8">
            ...
            >
              ...
              <span className="text-sm text-black ">Viết ra đánh giá của bạn về sản phẩm</span>
              <Icons.ArrowRight2 className="ml-auto icon" />
            </button>
            <p className="mt-6 text-xs text-gray-500">
              Sau khi gửi xác nhận, bạn sẽ nhận được các điểm quà tặng.
            </p>
          </div>
          ...
          >
            <div className="py-3 space-y-4 divide-y-2 lg:px-6 sm:px-2">
              {data?.data?.reviews?.map(item => <ReviewProductCard item={item} key={item.id} />)}
            </div>
          </ShowWrapper>
